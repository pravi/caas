...
public class TestResultStore {
    ...
    public boolean putPeriodicTestResults(String domain, List<Result> results, int iterationNumber) {
        addToCurrentResults(domain, results);
        addToPeriodicResults(domain, results, iterationNumber);
        return true;
    }

    public boolean putIteration(Iteration iteration) {
        synchronized (this.database) {
            try (Connection con = this.database.open()) {
    ...
    private boolean addToPeriodicResults(String domain, List<Result> results, int iterationNumber) {
        synchronized (this.database) {
            try (Connection con = this.database.beginTransaction(java.sql.Connection.TRANSACTION_SERIALIZABLE)) {
                Query query = con.createQuery("insert or replace into periodic_tests(domain,test,success,iteration_number) values(:domain,:test,:success,:iteration)");
                results.forEach(result -> {
                    query
                            .addParameter("test", result.getTest().short_name())
                            .addParameter("success", result.isSuccess())
